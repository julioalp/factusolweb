...
?>
<h3 align="center">Confirmaci&oacute;n del pedido</h3>
...
$ssql = 'SELECT * FROM F_DIR WHERE CLIDIR=' . $_SESSION['cod_factusol'];
$rs = mysql_query($ssql, $conn);
if(mysql_num_rows($rs) !=0 ) {
	echo('<select name="entrega">');
	while($datos = mysql_fetch_array($rs)) { echo('<option value="' . $datos['CODDIR'] . '">' . $datos['DOMDIR'] . '</option>'); }
	echo('</select>');
}else{
...
//tengo que ver si tengo que pedir más datos.
switch ($datos['TIPFPA']) {
	case 0: //no solicito más datos
		break;
	case 1:
		echo'C.C.C.';
...
?>
		...
		<tr>
			<td>
				<p>OTROS</p>
        <p>Referencia: <input name="referencia" type="text" id="referencia" size="12" maxlength="12" /></p>
        <p>Observaciones: <input name="observaciones" type="text" id="observaciones" size="50" maxlength="50" /></p>
      	<p>Pedido Realizado por: <input name="pedidopor" type="text" size="40" maxlength="40" /></p>
			</td>
		</tr>
	</table>
	...
	if (mysql_num_rows($rs) != 0) {
		...
		if (mysql_num_rows($rs1) !=0 ) {
				...
				switch(iva($linea['ARTLPC'], $cliente)) {
					...
					case -1:
						if($linea['IINLPC'] !=0 ){
							...
							$base1 = (number_format($linea['TOTLPC'], 2, '.', '') / (1 + $iva / 100));
						}else{
							$base1 = number_format($linea['TOTLPC'], 2, '.', '');
						}
						$neto1 += $base1;
						$desc1 += ($base1 * $datos['PDTO1PCL'] / 100);
						$ppago1 += ($base1 * $datos['PPPA1PCL'] / 100);
						$finan1 += ($base1 * $datos['PFIN1PCL'] / 100);
						$base1 += $base1;
						if(recargo($_SESSION['cod_factusol'])) { $re1 += ($base1 * $datos['PREC1PCL'] / 100); }
						break;
					case 0:
						if($linea['IINLPC'] != 0) {
							$iva = ivaart($linea['ARTLPC']);
						...
						$neto1 += $base;
						$desc1 += ($base * $datos['PDTO1PCL'] / 100);
						$ppago1 += ($base * $datos['PPPA1PCL'] / 100);
						$finan1 += ($base * $datos['PFIN1PCL'] / 100);
						$base1 += ($base - ($base * $datos['PDTO1PCL'] / 100) - ($base * $datos['PPPA1PCL'] / 100) + ($base * $datos['PFIN1PCL'] / 100));
						$iva1 += (($base - ($base * $datos['PPPA1PCL'] / 100)) * $datos1['PIV1CFG'] / 100);
						if(recargo($_SESSION['cod_factusol'])) { $re1 += ($base * $datos['PREC1PCL'] / 100); }
						break;
					case 1:
						if($linea['IINLPC'] !=0 ) {
							$iva = ivaart($linea['ARTLPC']);
						...
						$neto2 += $base;
						$desc2 += ($base * $datos['PDTO2PCL'] / 100);
						$ppago2 += ($base * $datos['PPPA2PCL'] / 100);
						$finan2 += ($base * $datos['PFIN2PCL'] / 100);
						$base2 += ($base - ($base * $datos['PDTO2PCL'] / 100) - ($base * $datos['PPPA2PCL'] / 100) + ($base * $datos['PFIN2PCL'] / 100));
						$iva2 += (($base - ($base * $datos['PPPA2PCL'] / 100)) * $datos1['PIV2CFG'] / 100);
						if(recargo($_SESSION["cod_factusol"])) { $re2 = $re2 + ($base * $datos["PREC2PCL"] / 100); }
						break;
					case 2:
						if($linea['IINLPC'] != 0) {
							$iva = ivaart($linea['ARTLPC']);
						...
						$neto3 += $base;
						$desc3 += ($base * $datos['PDTO3PCL'] / 100);
						$ppago3 += ($base * $datos['PPPA3PCL'] / 100);
						$finan3 += ($base * $datos['PFIN3PCL'] / 100);
						$base3 += ($base - ($base * $datos['PDTO3PCL'] / 100) - ($base * $datos['PPPA3PCL'] / 100) + ($base * $datos['PFIN3PCL'] / 100));
						$iva3 += ($base * $datos1['PIV3CFG'] / 100);
						if(recargo($_SESSION['cod_factusol'])) { $re3 += ($base * $datos['PREC3PCL'] / 100); }
						break;
				}//end switch    
			}//end while
			//formateo las cantidades a 2 decimales
			...
			if(iva($linea['ARTLPC'], $cliente) != -1) {
				//no exento de iva
				if ($base1 != 0) {
					echo('<tr bordercolor="#999999"><td align="right">');
				...
				echo('<tr bordercolor="#999999"><td>Exento</td><td align="right">');
				printf("%.2f", $desc1);
				echo('</td><td align="right">');
				printf("%.2f", $ppago1);
				echo('</td><td align="right">');
				...
				echo('</td><td align="right">');
				printf("%.2f", $base1);
				echo('</td><td align="right"></td><td align="right">');
				printf("%.2f", $re1);
				echo('</td><td align="right">');
				printf("%.2f", $base1 + $re1);
				echo('</td></tr>');
			...
			echo('<tr align="right" bordercolor="#999999">
						<td bgcolor="#999999" colspan="6"><strong>Total:</strong></td>
						<td bgcolor="#999999" colspan="2"><strong>');
			printf("%.2f", $base1 + $base2 + $base3 + $iva1 + $iva2 + $iva3 + $re1 + $re2 + $re3);
			echo('</strong>
						</td>
						</tr>
						</table>');
			//escribo son campos ocultos con la serie y el numero de factura para pasarselo a la pagina de confirmación
			echo('<input type="hidden" name="serie" value="' . $datos['TIPPCL'] . '" />
						<input type="hidden" name="numero" value="' . $datos['CODPCL'] . '" />');
		}
	}
}   
function tip_iva_art($articulo) {
	$conn = mysql_connect(BD_HOST, BD_USERNAME, BD_PASSWORD);
	mysql_select_db(BD_DATABASE1);
	$ssql = 'SELECT * FROM F_ART WHERE CODART=\'' . $articulo . '\'';
	$rs = mysql_query($ssql, $conn);
	$datos = mysql_fetch_array($rs);
	return $datos['TIVART'];
}  
function ivaart($articulo) {
	$conn = mysql_connect(BD_HOST, BD_USERNAME, BD_PASSWORD);
	mysql_select_db(BD_DATABASE1);
	$ssql = 'SELECT * FROM F_CFG';
	$rs = mysql_query($ssql, $conn);
	$datos = mysql_fetch_array($rs);
	$tipiva = tip_iva_art($articulo);      
	switch($tipiva) {
		case 0:
...
function iva($articulo, $cliente) {
	//primero compruebo el el cliente está o no exento de iva si está exento devuelvo -1
	$conn = mysql_connect(BD_HOST, BD_USERNAME, BD_PASSWORD);
	mysql_select_db(BD_DATABASE1);
	$ssql = 'SELECT * FROM F_CLI WHERE CODCLI=' . $cliente;
	$rs = mysql_query($ssql, $conn);
...
?>
	<tr bordercolor="#cccccc" bgcolor="#ffffff">
		<td bgcolor="<?=$colorcelda?>"><?=$linea['ARTLPC']?></td>
		<td bgcolor="<?=$colorcelda?>"><?=$linea['DESLPC']?></td>
		<td align="right" bgcolor="<?=$colorcelda?>">
<?php
				//el precio va ya sin iva asi que compruebo que no tenga iva
				if($linea['IINLPC'] == 0) {
					if(iva($linea['ARTLPC'], $cliente) == '-1') {
						$iva = ivaart($linea['ARTLPC']);
				...
				printf("%.2f", $base);
				echo('</td>');
				echo('<td align="right" bgcolor="' . $colorcelda . '">' . decimal($linea['CANLPC']) . '</td>');
			...
			echo('<td colspan="6">');
			escribir_bases($_SESSION['cod_factusol']);
			echo('</td>');
			echo('</tr>');
		}else{
?>
	<tr bordercolor="#cccccc" align="center" bgcolor="#ffffff">
		<td colspan="6" align="center">NO SE HAN ENCONTRADO ARTICULOS</td>
	</tr>
	<tr bordercolor="#999999" align="center">
		<td colspan="4"><font color="#000000" face="Verdana" size="2"><b>Total:</b></font></td>
		<td>&nbsp;</td>
		<td>&nbsp;</td>
	</tr>
</table>
...
?>
	<tr bordercolor="#cccccc" align="center" bgcolor="#ffffff">
		<td colspan="6" align="center">NO SE HA ENCONTRADO PEDIDO EN CURSO</td>
	</tr>
